...
: consumers never import from ``transport/``, ``protocol/`` (the
...
real one in tests without a serial port or a machine.

Async contract: every coroutine here is cancellable at any
...
``soft_reset`` (which is always available,).

Deviation from the sketch: ``send_program`` is typed as a plain ``def``
...
``async for progress in controller.send_program(...)`` directly. The sketch
...
class Controller(Protocol):
    ...
    async def soft_reset(self) -> None:
        ...
        SAFETY INVARIANT: always available regardless of state
        or queue depth. Bypasses the streamer; clears the ack queue and modal
        state on the resulting ``Welcome``.
        """
        ...
    ...
    async def home(self, axes: Iterable[Axis] | None = None) -> None:
        ...
        Raises:
            MachineNotReadyError: the machine is in ``Door`` state.
    ...
    async def read_settings(self) -> Settings:
        """Read ``$$`` and return the parsed settings map.
    ...
    async def write_setting(self, key: int, value: str) -> None:
        """Write ``$key=value`` and verify it by re-reading ``$$``.

        Raises:
            SettingsMismatchError: the read-back value did not match.
            NotConnectedError: no open connection.
        """
        ...

    def send_program(self, lines: AsyncIterable[str]) -> AsyncIterator[ProgramProgress]:
        """Stream a program, yielding progress as lines are sent and acked.

        The returned async iterator drives the character-counting streamer
; cancelling iteration stops sending but does not stop the
        machine — follow with ``soft_reset`` to halt.
    ...
    def status_stream(self) -> AsyncIterator[Status]:
        """Yield status reports as they arrive (default 10 Hz polling,,).

        Continues for the life of the connection. Loss of consecutive reports
        is surfaced by the controller as a disconnect, not swallowed
        here.
        """
        ...
