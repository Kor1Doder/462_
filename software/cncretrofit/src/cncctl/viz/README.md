...
the design notes originally named `pyGCodeDecode` (primary) and
...
- **Heavy native dependency tree:** installing it pulls `vtk` (the C++
  ...
  deployment target** this is a serious liability: vtk aarch64
...
- **3D-printer-oriented API:** machine configs are *printer presets*
  ...
  — exactly the "3D-printing-oriented API gets in the way" risk the project docs
  flagged.

### `gcode-simulator` (the documented fallback) — unusable
- Its CLI/model is **X/Y only**. A 3-axis mill needs **Z** to bound plunge
  depth for the soft-limit check, so the fallback is a dead end.

### Decision
The safety-critical part — the soft-limit **bounding box** — is **pure geometry**, computable directly from the M6 `Program`.
...
- `analyze.py` — bounding box, per-axis min/max, total travel, duration, and
  soft-limit violations. Pure Python. This is the safety check.
