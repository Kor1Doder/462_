...
class TransportConfig(msgspec.Struct, frozen=True):
    default_port_windows: str
    default_port_linux: str
    baudrate: int = 115200
...
def save_config(config: Config, path: Path) -> None:
    ...
    lines = [
        ...
        "",
        "[transport]",
        f"default_port_windows = {_toml_str(config.transport.default_port_windows)}",
        f"default_port_linux = {_toml_str(config.transport.default_port_linux)}",
...
def default_port(config: Config) -> str:
    ...
    # branches genuinely run, one per OS.
    current_platform = sys.platform
    if current_platform == "win32":
        return config.transport.default_port_windows
    return config.transport.default_port_linux
