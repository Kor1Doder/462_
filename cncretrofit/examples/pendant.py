...
Drive the real machine (table clear, spindle off, e-stop in hand!):
    ...
    uv run python examples/pendant.py --port COM3
"""
...
class _Keyboard:
    """Read single keypresses without Enter, cross-platform."""

    def __enter__(self) -> _Keyboard:
        self._restore = None
        if sys.platform != "win32":
            # POSIX: cbreak mode reads one char at a time but keeps Ctrl-C working.
            import termios
            import tty
            ...
            self._restore = lambda: termios.tcsetattr(self._fd, termios.TCSADRAIN, self._old)
        return self
