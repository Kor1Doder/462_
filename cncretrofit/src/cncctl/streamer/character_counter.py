"""Character-counting streamer (CLAUDE.md §5.1, §7 M4, §8.4).
...
:1152 and the send condition at :1247). We follow CLAUDE.md §5.1's canonical
...
* no safety margin. ioSender sends against ``0.9 * buffer`` as a hardware-
  handshake high-water mark; CLAUDE.md §8.4 forbids "probably fine" margins, so
  we account precisely against the real buffer size (verify via ``$I``, §5.1).

The accounting (:class:`BufferAccount`) is pure and synchronous so the §8.4
...
def line_cost(line: str) -> int:
    """Exact wire-byte cost of ``line`` including its terminator (§5.1)."""
...
class BufferAccount:
    ...
    oldest (each ``ok``/``error`` acks the oldest line, §5.1). Every method keeps
    ...
    def can_send(self, cost: int) -> bool:
        """Whether a line of ``cost`` bytes fits right now (§5.1 condition)."""
    ...
    def reserve(self, line: str, cost: int) -> None:
        ...
        Raises:
            BufferOverflowError: if sending would exceed the buffer (§8.4). With
    ...
    def clear(self) -> None:
        """Drop all outstanding accounting (after a device reset / welcome, §5.4)."""
        self._pending.clear()
        self._outstanding = 0
...
class CharacterCountingStreamer:
    """Streams lines to a send callback, never overflowing the RX buffer (§8.4).
    ...
    def reset(self) -> None:
        """Drop outstanding accounting after a device reset (§5.4).
    ...
    async def stream(self, lines: AsyncIterable[str]) -> AsyncIterator[StreamProgress]:
        ...
        caller must follow with a soft reset (§8.3, §8.6).

        Raises:
            BufferOverflowError: a single line is larger than the whole RX
                buffer and can never be sent (§8.4).
        """
        sent = 0
